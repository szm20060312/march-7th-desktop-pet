...
pub fn setup<R: Runtime>(
    ...
    let menu = MenuBuilder::new(app)
        ...
        .text("hide", "隐藏");
    #[cfg(target_os = "macos")]
    let menu = menu.text("move-to-current-space", "切换到当前桌面");
    let menu = menu
...
#[cfg(target_os = "macos")]
fn move_to_current_space<R: Runtime>(app: &AppHandle<R>) -> Result<(), String> {
    let desktop = app.state::<Desktop<R>>();
    {
        let mut session = desktop.shared.session.lock().unwrap();
        if session.begin_space_switch(Instant::now()).is_none() {
            return Ok(());
        }
    }
    if let Err(error) = main_window(app).and_then(|window| window.hide().map_err(|e| e.to_string()))
    {
        desktop.shared.session.lock().unwrap().cancel_space_switch();
        return Err(error);
    }
    desktop.shared.wake.notify_one();
    Ok(())
...
fn handle_menu_event<R: Runtime>(app: &AppHandle<R>, event: MenuEvent) {
    ...
    let result = match operation {
        ...
        #[cfg(target_os = "macos")]
        "move-to-current-space" => move_to_current_space(app),
...
fn worker<R: Runtime>(app: AppHandle<R>, shared: Arc<Shared>, mut store: Option<Store>) {
    ...
    loop {
        let (save, quit, poll, scale, startup, space_switch) = {
            let mut session = shared.session.lock().unwrap();
            loop {
                if let Some((save, code)) = session.quit.take() {
                    break (save, Some(code), false, None, false, None);
                }
                let now = Instant::now();
                ...
                let space_switch = session.take_space_switch_due(now);
                if startup {
                    session.startup_due = None;
                }
                if save.is_some() || poll || scale.is_some() || startup || space_switch.is_some() {
                    break (save, None, poll, scale, startup, space_switch);
                }
        ...
        if let Some(generation) = space_switch {
            let handle = app.clone();
            if let Err(error) = app.run_on_main_thread(move || {
                ...
                if desktop
                    ...
                    .lock()
                    .unwrap()
                    .space_switch_is_current(generation)
                {
                    if let Err(error) = show_window(&handle) {
                        report_failure("move-to-current-space", error);
                    }
                }
            }) {
                report_failure("queue-move-to-current-space", error);
            }
        }
