...
自动提醒呈现仍遵守这一边界：Rust 的单一 Bubble 协调器确认自动展示窗口已显示后，才向主角色窗口发送一次带展示 ID 与提醒类型的事件；手动待处理和专注完成不发送。TypeScript 从内置角色目录选动作与短句，喝水时从独立 2×2 透明动作图集播放递杯四帧并保持末帧，直到原生发送同 ID 的结束事件。短句只进入主角色原有点击对话框。自动展示窗口默认全透明并忽略鼠标，点击角色时主窗口以展示 ID 请求 Rust 打开旁边的独立小操作窗；Rust 验证仍为当前自动展示后才开放“接过水杯／待会儿”。多项合并的主操作打开手动待处理列表。手动待处理与专注完成继续使用原有列表卡，角色隐藏或穿透时从托盘恢复。前端不接管业务计时，窗口隐藏或角色切换不改变待处理状态；原生透明、焦点、跨屏位置和首次点击仍需双平台实测。
