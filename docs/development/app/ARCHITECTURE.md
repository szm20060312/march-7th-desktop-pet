...
所有者试用反馈后的提醒呈现仍遵守这一边界：Rust 的单一 Bubble 协调器确认自动提醒对白实际显示后，才向主角色窗口发送一次带展示 ID 与提醒类型的事件；手动待处理和专注完成不发送该事件。TypeScript 从内置角色目录选动作与短句，独立透明对白窗口订阅同一选角快照并保留原有命令按钮。前端不接管业务计时，窗口隐藏或角色切换不改变待处理状态；原生透明、焦点和跨屏位置仍需双平台实测。
