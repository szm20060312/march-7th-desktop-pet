...
自动提醒呈现仍遵守这一边界：Rust 的单一 Bubble 协调器选中当前自动提醒后，向可见主角色窗口发送一次带展示 ID 与提醒类型的事件；手动待处理和专注完成不发送。TypeScript 从内置角色目录选动作与短句，喝水时从独立 2×2 透明动作图集播放递杯四帧并保持末帧，直到原生发送同 ID 的结束事件；主图集与递杯图集通过两层角色 DOM 淡入淡出，避免硬切。短句只进入主角色原有点击对话框。未点击角色时**不显示独立提醒窗口**；点击后主窗口以展示 ID 请求 Rust 延长当前自动展示并打开旁边的双操作窗，服务端再给 60 秒操作时间。多项合并的主操作打开手动待处理列表。手动待处理与专注完成继续使用原有列表卡，角色隐藏或穿透时从托盘恢复。前端不接管业务计时，窗口隐藏或角色切换不改变待处理状态；点开后的透明、焦点、跨屏位置和首次点击仍需双平台实测。
