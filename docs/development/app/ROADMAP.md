...
- [草稿 PR #27](https://github.com/szm20060312/march-7th-desktop-pet/pull/27) 以 `main@cbf5753`（含 PR #12）为基线，集成 G7 开发栈及 M5-A/B/C 可独立完成的技术部分；尚未合入 `main` 或正式发布。稳定版本仍为 v0.2.0。其功能提交 `4cb11dd29cfa152571a743833ba7a09ea9e939b4` 的 Windows/macOS 原生回归包与未签名安装候选已在[同一次 CI](https://github.com/szm20060312/march-7th-desktop-pet/actions/runs/35852829974)生成并通过静态核对；双平台实机验收仍未完成。源码范围与缺口见 [M5 技术候选](M5-TECHNICAL-CANDIDATE.md)。后续提交须以自己的完整提交号重新核对构建证据。
...
当前入口是**对已生成的同提交测试包进行集中实机回归**。旧 M0 双平台测试产物曾交付并静态校验，见 [M0 构建记录](M0-BUILD-REPORT.md)，但不能代替新架构加 M5 功能后的回归。所有者负责 Windows、协作者负责 Mac；未完成两平台观察、两周试用与价值复盘前，M0/M1/M2/M3/M4/M5 均不能因源码或自动检查通过而关闭。
