...
更新：2026-09-22。产品范围和 Goal 契约以 [PRODUCT-PLAN.md](PRODUCT-PLAN.md) 为准；本页只负责依赖导航，[TODO.md](TODO.md) 记录实际状态。

## 当前起点

- 本轮基线：`main@77d1241`，内容与架构提交 `da56984` 一致；独立 App 仍为 v0.2.0。
- 重构提交的 Windows/macOS CI 已通过，见 [验证记录](ARCHITECTURE-VALIDATION.md)。
- 待机、注视和拖动已实现；本次架构的双平台 GUI 回归尚未完成，M0 未关闭。
- 托盘、穿透、位置持久化、第二角色、点击动作、短句和提醒尚未实现。
...
当前入口是 G0 的实机回归。双平台同提交测试产物已经交付并静态校验，见 [M0 构建记录](M0-BUILD-REPORT.md)；所有者和协作者尚需完成真实回归。用户后续要求持续推进至 M5-A/B/C 中可独立完成的工作，允许在隔离分支准备后续实现，但 M0/M1 等阶段仍须取得相应验收证据才能关闭。
