...
- macOS 状态栏显示／隐藏／退出原型已在 `macos/v0.3-desktop-controls` 实现；Windows 对齐、穿透、位置持久化、第二角色、点击动作、短句和提醒尚未实现。
...
候选下一目标是 G0。它需先提供双平台同提交测试产物，再由所有者和协作者完成实机回归。当前仅保存计划，尚未创建或执行 G0；不进入 M1、不生成第二角色、不实现提醒。
