...
- 每次提醒建窗注入新 windowToken，ready 校验原生窗口身份、当前 token、presentationId 与退出状态。提前到达的 ready 保留；两个实际位置/尺寸/缩放观测稳定且 ready 后才显示。同 ID 不重复 show，新 ID 先隐藏旧视图；自动 null、原生关闭、销毁、重载与退出使旧任务失效。正常自动约 20 秒收起仍完全由后端驱动。
