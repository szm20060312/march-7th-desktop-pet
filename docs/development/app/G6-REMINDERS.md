...
2026-09-23 角色引导候选：根据 Windows 所有者反馈，自动到期提醒改为当前角色挥手并说一句对应提醒的短句，旁边显示带头像和“完成／稍后”的贴身对白。对白仍使用原有独立可交互窗口，以保证角色穿透时按钮可点；只改变呈现，不改变 Rust 的到期、待处理、稍后、暂停或自动收起规则。窗口宽 320 逻辑像素，高度按 0/1、2、3 项分别为 220、260、300；过小工作区仍约束并允许滚动。隐藏角色时对白仍能显示角色身份，主角色不在隐藏状态下播放动作。手动查看待处理不触发新的角色到期动作。当前仅有本地自动与浏览器布局证据，双平台原生焦点、透明、位置和真实“不打扰”感尚待实机验收。
