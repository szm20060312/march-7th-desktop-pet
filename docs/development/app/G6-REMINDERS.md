...
`fbd444d` Windows 所有者复测确认：未点击时黑框消失，点击后操作正常；但对白和操作停留仍偏短。所有者明确选择“继续自动收起，约 2 分钟”，因此下一候选把未点击展示和点击后操作宽限都设为 120 秒。未回应自动收起仍保留待处理，不自动重播；该时长调整尚待新包实测。
...
- 每次确需提醒建窗时注入新 windowToken，ready 校验原生窗口身份、当前 token、presentationId 与退出状态。未点击的自动提醒不建可见窗口；点角色后或手动查看时才按需创建。提前到达的 ready 保留；两个实际位置/尺寸/缩放观测稳定且 ready 后才显示。同 ID 不重复 show，新 ID 先隐藏旧视图；自动 null、原生关闭、销毁、重载与退出使旧任务失效。正常自动约 2 分钟收起，点开操作后由 Rust 从点击时再给予约 2 分钟。
