...
- 自动提醒由可见角色动作及原有小对话框呈现，未点击时不创建可见的独立空窗口；点击角色后才开放独立、可交互的小操作窗。角色隐藏或穿透时，从托盘手动“查看待处理”进入完整列表；自动展示不抢焦点。设置窗口只因用户主动打开获取焦点。手动列表保留逐项“完成”、全局“稍后提醒”和收起；自动提示约 2 分钟后收起，点击后由 Rust 从点击时给当前展示约 2 分钟操作时间。消息文本不含内部实现词。
...
- presentation、revision、单调截止和保存状态是会话数据，不进文件。自动展示 120 秒；点击角色后服务端只对当前自动展示 ID 把截止延到点击后 120 秒。墙钟或单调截止任一到达即收起；加入新项保持原 presentation ID 和截止。角色提醒台词在展示期间持续可见，结束后递杯按四帧倒序收回，主图集与递杯图集淡入淡出，约 880 ms 后清除台词；这是表现收尾，不延长业务展示或操作资格。重启不恢复旧 presentation，已处理 pending 仍待处理而不重复自动提示。
