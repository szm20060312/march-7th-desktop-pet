...
包含 G7-B1 的构建在同一配置目录只允许一个采用新协议的可写实例，第二次启动正常结束，不唤醒已有窗口。升级前仍须先退出旧版。`instance.lock` 会保留，文件存在不表示程序仍运行，不要删除它来解锁。目录或锁不可用时三服务统一降级：临时角色选择、默认关闭且只读的提醒、位置不可保存；不会换目录或覆盖原配置。对应验证见共用清单 G7-B1 段，自动子进程锁测试不替代双平台 GUI。
