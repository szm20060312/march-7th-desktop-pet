...
这个包用于验证架构重构与本分支 G1/G2 的真实桌面行为。应用版本号仍为 v0.2.0，但本分支已加入托盘显示/隐藏/重置/退出、交互/点击穿透及位置保存；并已加入两角色互动、托盘切换和独立选择保存；提醒尚未实现。构建成功不代表实机验收通过，M0/M1/M2 与 G4 仍待验收，必须以 sourceCommit 区分测试基线。
